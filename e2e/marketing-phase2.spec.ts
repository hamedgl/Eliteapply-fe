...
const requiredRoutes = [
  ...
  [
    ...
    "Keep every scholarship application moving.",
  ],
  [
  ...
  [
    ...
    "Keep every document connected to the requirement it supports.",
  ],
  [
    "/features/reference-tracking",
    "Track reference requirements before they become urgent.",
  ],
  [
    "/features/submission-readiness",
    "Know what is ready, what is missing and what needs one final review.",
  ],
  [
    "/how-it-works",
    "From opportunity to final check, keep the whole application connected.",
  ],
  ...
  [
    ...
    "Organise every scholarship application without losing the details.",
  ],
  [
    "/scholarship-deadline-tracker",
    "Turn scholarship deadlines into a preparation plan.",
  ],
  [
    "/scholarship-application-checklist",
    "Build a scholarship application checklist around the real requirements.",
  ],
] as const;
...
const guideRoutes = [
  ...
  [
    ...
    "How to preserve your authentic voice when using AI assistance",
  ],
  [
...
test("high-intent routes target distinct search questions", async ({
  page,
}) => {
  const titles = new Set<string>();
  const headings = new Set<string>();
  for (const [path] of requiredRoutes.filter(([path]) =>
    ...
    await expect(
      page.getByRole("heading", {
        name: "Tracker, organiser, deadline tracker or checklist?",
      }),
    ).toBeVisible();
  }
...
test("homepage and guide links create contextual journeys", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("heading", { name: "Continue with practical guidance" }),
  ).toBeVisible();

  await page
    .getByRole("link", {
      name: "How to organise multiple scholarship applications",
    })
    .click();
  ...
  await page
    .getByRole("link", { name: "Open the related EliteApply capability" })
    .click();
  await expect(page).toHaveURL(/\/scholarship-application-organiser$/);
});

test("resource hub contains four useful clusters and provider-first guidance", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(
      "always defer to the scholarship provider's current instructions",
    ),
  ).toBeVisible();
  await expect(
    page.getByRole("link", {
      name: "How to preserve your authentic voice when using AI assistance",
    }),
  ).toBeVisible();
});
...
test("pricing and legal routes publish specific controls without unsupported claims", async ({
  ...
  await expect(
    page.getByText(
      "No paid plan is currently enabled in this environment. Free access remains available.",
    ),
  ).toBeVisible();
  await expect(
    page.getByText("Start without entering payment details."),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(/bank-level|SOC 2 certified|military-grade/i),
  ).toHaveCount(0);
...
test("representative Phase 2 routes reflow without horizontal page overflow", async ({
  page,
}) => {
  const routes = [
    "/features/scholarship-application-tracker",
