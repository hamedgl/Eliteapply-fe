...
export const adminApi = {
  readiness: () =>
    apiRequest<Record<string, unknown>>("/admin/launch-readiness"),
  gates: () => apiRequest<Record<string, unknown>[]>("/admin/launch-gates"),
  updateGate: (key: string, body: S["LaunchGateUpdate"]) =>
    apiRequest<Record<string, unknown>>(`/admin/launch-gates/${e(key)}`, {
      method: "PUT",
      body,
    }),
  updateFlag: (key: string, body: S["FeatureFlagUpdate"]) =>
    apiRequest<Record<string, unknown>>(`/admin/feature-flags/${e(key)}`, {
      method: "PUT",
      body,
    }),
  flags: () => apiRequest<S["FeatureFlagSummary"][]>("/admin/feature-flags"),
  operations: (filters: {
    type?: S["OperationSummary"]["type"];
    status?: string;
    cursor?: string | null;
  } = {}) =>
    apiRequest<S["CursorPage_OperationSummary_"]>(
      `/admin/operations${qs({ ...filters, limit: 25 })}`,
    ),
  operation: (id: string) =>
    apiRequest<S["OperationDetail"]>(`/admin/operations/${e(id)}`),
  retryOperation: (id: string) =>
    apiRequest<S["OperationDetail"]>(`/admin/operations/${e(id)}/retry`, {
      method: "POST",
    }),
  cancelOperation: (id: string) =>
    apiRequest<S["OperationDetail"]>(`/admin/operations/${e(id)}/cancel`, {
      method: "POST",
    }),
  redrive: (queue: string, body: S["DlqRedriveRequest"]) =>
    apiRequest<S["DlqRedriveResponse"]>(`/admin/queues/${e(queue)}/redrive`, {
      method: "POST",
      body,
    }),
  userSearch: (email?: string, userId?: string) =>
    apiRequest<S["UserSearchResult"]>(
      `/admin/users/search${qs({ email, user_id: userId })}`,
    ),
  userEntitlement: (id: string) =>
    apiRequest<S["EntitlementResponse"]>(`/admin/users/${e(id)}/entitlement`),
  userUsage: (id: string, cursor?: string | null) =>
    apiRequest<S["UsageResponse"]>(
      `/admin/users/${e(id)}/usage-ledger${qs({ cursor, limit: 25 })}`,
    ),
  pendingCatalogue: (kind: string, cursor?: string | null) =>
    apiRequest<S["CursorPage_CatalogueModerationItem_"]>(
      `/admin/catalogue/${e(kind)}/pending${qs({ cursor, limit: 25 })}`,
    ),
  approveCatalogue: (kind: string, id: string) =>
    apiRequest<S["CatalogueModerationItem"]>(
      `/admin/catalogue/${e(kind)}/${e(id)}/approve`,
      { method: "POST" },
    ),
  rejectCatalogue: (kind: string, id: string, body: S["CatalogueRejectRequest"]) =>
    apiRequest<void>(`/admin/catalogue/${e(kind)}/${e(id)}/reject`, {
      method: "POST",
      body,
    }),
  mergeCatalogue: (kind: string, id: string, body: S["CatalogueMergeRequest"]) =>
    apiRequest<S["CatalogueModerationItem"]>(
      `/admin/catalogue/${e(kind)}/${e(id)}/merge`,
      { method: "POST", body },
    ),
  referenceEvents: (id: string, cursor?: string | null) =>
    apiRequest<S["CursorPage_ReferenceEventSummary_"]>(
      `/admin/references/${e(id)}/events${qs({ cursor, limit: 50 })}`,
    ),
  runDueReminders: () =>
    apiRequest<void>("/admin/reminders/run-due", { method: "POST" }),
};
